...
function preload(){
  font1 = loadFont("assets/KunstGroteskBETA-Bold.otf");
...
function setup() {
  ...
  wormToggle = createCheckbox();
  wormToggle.position(width/2-120, 30);
  ...
  saveButton.position(width/2+120, 30);
  saveButton.mousePressed(saveVector);
...
function draw() {
  ...
  noFill();
    stroke(255,alphaPreview); strokeWeight(2);
  ...
  if(wormState){
    for(var j = 0; j<particles.length; j++){
      for (var i = 0;  i<particles[j].length; i++){
        if(i>0){
          stroke(255); strokeWeight(partSizes[j]); noFill(); strokeJoin(ROUND);
  ...
  //   for (var i = 0;  i<particles[j].length; i++){
  //     if(i>0){
  //       stroke(0); strokeWeight(partSizes[j]/10); noFill(); strokeJoin(ROUND);
  //       line(particles[j][i].body.position.x,particles[j][i].body.position.y,particles[j][i-1].body.position.x,particles[j][i-1].body.position.y);
  //     } else {
  //       stroke(0); strokeWeight(partSizes[j]/10); noFill(); strokeJoin(ROUND);
  //       line(particles[j][i].body.position.x,particles[j][i].body.position.y,particles[j][i].body.position.x,particles[j][i].body.position.y);
